...
func getRegexesPath() string {
	...
	return filepath.Join(filepath.Dir(filename), "..", "..", "regexes", "client", "feed_readers.yml")
}
...
func getFixturesPath() string {
	...
	return filepath.Join(filepath.Dir(filename), "fixtures", "feed_reader.yml")
}
...
// TestParse mirrors DeviceDetector/Tests/Parser/Client/FeedReaderTest.php.
...
// TestStructureFeedReaderYml mirrors FeedReaderTest::testStructureFeedReaderYml.
func TestStructureFeedReaderYml(t *testing.T) {
	...
	for _, item := range items {
		for _, k := range []string{"regex", "name", "version", "url"} {
			v, ok := item[k]
