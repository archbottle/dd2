...
func NewParserFactory(regexesPath string, opts ...common.FactoryOption) (*ParserFactory, error) {
	...
	db, err := common.NewYAMLListDB(f.patterns, func(e *Entry, compiler *common.RegexCompiler) error {
		...
		if err != nil {
			return fmt.Errorf("compiling mediaplayer pattern (%s): %w", e.Name, err)
		}
		e.compiled = re
		return nil
	}, opts...)
...
// NewDefaultParserFactory creates a factory using the repo-local mediaplayers.yml path.
func NewDefaultParserFactory() (*ParserFactory, error) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		return nil, fmt.Errorf("failed to get caller info")
	}
	regexesPath := filepath.Join(filepath.Dir(filename), "..", "..", "regexes", "client", "mediaplayers.yml")
