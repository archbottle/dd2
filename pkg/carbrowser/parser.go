...
// Parser parses a single user agent for car browser information.
...
// Parse detects car browser devices and returns a Match, or nil if not detected.
// Mirrors PHP: DeviceDetector\Parser\Device\CarBrowser::parse(): ?array
...
func (p *Parser) matchEntry(e *Entry) *Match {
	...
	deviceType := e.Device
	if deviceType == "" {
		deviceType = "car browser"
	}

	// Brand-level model (rare; not used in current YAML but supported for parity).
	if e.Model != "" {
		return &Match{
			Type:  deviceType,
			Brand: brand,
	...
	for i := range e.Models {
		...
		// Model rules can override brand/device (for parity with AbstractDeviceParser).
		if m.Brand != "" {
			brand = m.Brand
		}
		if m.Device != "" {
