...
type ParserFactory struct {
	...
	// Pre-match for Notebook.php: matchUserAgent('FBMD/')
	fbmd common.UniversalRegex
...
// NewParserFactory loads and compiles the notebook regex DB from YAML.
func NewParserFactory(regexesPath string, opts ...common.FactoryOption) (*ParserFactory, error) {
	...
	entries, err := parseNotebooksYAMLOrdered(data)
	if err != nil {
		return nil, fmt.Errorf("parsing notebooks YAML: %w", err)
	}
	...
	if err := f.compileFBMD(compiler, cfg.RegexMode); err != nil {
		return nil, err
	}
...
func NewDefaultParserFactory() (*ParserFactory, error) {
	...
	regexesPath := filepath.Join(filepath.Dir(filename), "..", "..", "regexes", "device", "notebooks.yml")
...
func (f *ParserFactory) compileFBMD(compiler *common.RegexCompiler, regexMode common.RegexMode) error {
	wrapped := common.WrapDeviceDetectorPattern("FBMD/")
	re, err := compiler.Compile(wrapped)
	if err != nil {
		if regexMode == common.Re2Only {
			return fmt.Errorf("compiling FBMD precheck (RE2-only mode): %w", err)
		}
		return fmt.Errorf("compiling FBMD precheck: %w", err)
	}
	f.fbmd = re
	return nil
}

func parseNotebooksYAMLOrdered(data []byte) ([]*Entry, error) {
	var root yaml.Node
